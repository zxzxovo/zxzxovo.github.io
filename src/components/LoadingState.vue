  ...
  <div class="loading-container" :class="containerClass">
    <div class="loading-content">
      ...
      <template v-if="type === 'skeleton'">
        <div class="space-y-4">
          <div
            class="skeleton-line h-8 bg-gray-200 dark:bg-gray-700 rounded"
          ></div>
          <div
            class="skeleton-line h-4 bg-gray-200 dark:bg-gray-700 rounded w-3/4"
          ></div>
          <div
            class="skeleton-line h-4 bg-gray-200 dark:bg-gray-700 rounded w-1/2"
          ></div>
        </div>
      </template>
      ...
      <template v-else-if="type === 'pulse'">
        ...
        <p
          v-if="text"
          class="text-center mt-3 text-gray-600 dark:text-gray-400"
        >
          {{ text }}
        </p>
      </template>
...
const containerClass = computed(() => {
  ...
  if (props.fullscreen) {
    ...
    classes.push(
      "absolute inset-0 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm",
    );
  } else {
    classes.push("py-8");
  }
