  ...
  md.inline.ruler.before("text", "snack_text", (state, silent) => {
    ...
    if (start > 0) {
      ...
      if (prevChar === 0x0a /* \n */ || prevChar === 0x0d /* \r */) {
        if (
          start + 1 < max &&
          state.src.charCodeAt(start + 1) === 0x20 /* space */
        ) {
          return false;
        }
      }
...
export async function processMarkdownContent(
  ...
  try {
    ...
    const styledHtml = html
      .replace(
        /<h1([^>]*)>/g,
        '<h1$1 class="text-3xl font-bold text-gray-900 dark:text-white mt-8 mb-4 pb-2 border-b border-gray-200 dark:border-gray-700 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<h2([^>]*)>/g,
        '<h2$1 class="text-2xl font-semibold text-gray-900 dark:text-white mt-6 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<h3([^>]*)>/g,
        '<h3$1 class="text-xl font-semibold text-gray-900 dark:text-white mt-5 mb-2 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<h4([^>]*)>/g,
        '<h4$1 class="text-lg font-medium text-gray-900 dark:text-white mt-4 mb-2 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<h5([^>]*)>/g,
        '<h5$1 class="text-base font-medium text-gray-900 dark:text-white mt-4 mb-2 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<h6([^>]*)>/g,
        '<h6$1 class="text-sm font-medium text-gray-900 dark:text-white mt-4 mb-2 text-center cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors" onclick="updateUrlHash(this.id)">',
      )
      .replace(
        /<p>/g,
      ...
      .replace(
        /<blockquote>/g,
        '<blockquote class="border-l-4 border-blue-500 pl-4 py-2 my-4 bg-blue-50 dark:bg-blue-950/30 text-gray-700 dark:text-gray-300 italic">',
      )
      .replace(
        /<ul>/g,
        '<ul class="list-disc list-inside space-y-2 my-4 text-gray-700 dark:text-gray-300">',
      )
      .replace(
        /<ol>/g,
        '<ol class="list-decimal list-inside space-y-2 my-4 text-gray-700 dark:text-gray-300">',
      )
      .replace(/<li>/g, '<li class="ml-4">')
      .replace(
        /<a /g,
        '<a class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 underline" ',
      )
      .replace(
        /<code([^>]*)>/g,
        '<code$1 class="bg-gray-100 dark:bg-gray-800 text-purple-600 dark:text-purple-400 px-1 py-0.5 rounded text-sm font-mono">',
      )
      .replace(
        /<table>/g,
        '<div class="overflow-x-auto my-4"><table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg">',
      )
      .replace(/<\/table>/g, "</table></div>")
      .replace(/<thead>/g, '<thead class="bg-gray-50 dark:bg-gray-800">')
      .replace(
        /<th>/g,
        '<th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">',
      )
      .replace(
        /<td>/g,
        '<td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300 border-b border-gray-100 dark:border-gray-800">',
      )
      .replace(
        /<hr>/g,
...
export async function renderMarkdownWithTOC(
  content: string,
): Promise<{ html: string; toc: TableOfContent[] }> {
  try {
