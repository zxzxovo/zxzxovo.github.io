...
`poll_fn` 函数使用闭包创建一个 `Future` 实例。上面的代码片段创建了一个 `Delay` 实例，轮询它一次，然后将 `Delay` 实例发送到一个新任务中等待。在这个例子中，`Delay::poll` 被用**不同**的 `Waker` 实例调用多次。当这种情况发生时，您必须确保在传递给*最近*一次 `poll` 调用的 `Waker` 上调用 `wake`。
