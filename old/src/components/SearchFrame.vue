      ...
      >
        ...
        <div class="flex-1 overflow-y-auto p-6 search-results">
          ...
          <div v-if="isSearching" class="flex justify-center items-center py-8">
            <div
              class="animate-spin rounded-full h-8 w-8 border-b-2 border-violet-500"
            ></div>
          </div>
          ...
          >
            <svg
              class="mx-auto w-16 h-16 mb-4 text-zinc-300"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div v-else class="space-y-4">
            ...
            >
              <div class="flex items-start justify-between mb-2">
                <h3
                  class="text-lg font-semibold text-zinc-900 dark:text-white flex-1"
                >
                  {{ result.title }}
                </h3>
                <span
                  class="ml-2 px-2 py-1 text-xs rounded-full bg-violet-100 dark:bg-violet-900 text-violet-800 dark:text-violet-200"
                >
                  {{
                    result.type === "blog"
                      ? "博客"
              ...
              <!-- 博客文章的标签 -->
              <div
                v-if="result.type === 'blog' && result.tags"
              ...
              <!-- 合集的作者信息 -->
              <div
                v-else-if="result.type === 'book' && result.author"
              ...
              <!-- 章节的合集信息 -->
              <div
                v-else-if="result.type === 'chapter' && result.bookTitle"
                class="text-xs text-zinc-500 dark:text-zinc-400"
              >
                来自: {{ result.bookTitle }}
              </div>
        ...
        <!-- 底部提示 -->
        <div
          class="px-6 py-3 bg-zinc-50 dark:bg-zinc-700 text-xs text-zinc-500 dark:text-zinc-400 flex justify-between"
        >
          <span>按 ESC 键关闭</span>
